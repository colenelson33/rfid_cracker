...
# Default access bits (factory values) for Key A write-back
DEFAULT_ACCESS = [0xFF, 0x07, 0x80, 0x69]

def write_trailer_block(sector, keyA=None, keyB=None, access_bits=None, state=0):
    """
    Authenticate with current keyA from state.json, then write new Key A, access bits, Key B.
    """
    ...
    # Prepare block data: KeyA | access_bits | KeyB
    block_data = []
    if keyA:
